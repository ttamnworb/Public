...
{
	logDebug(address, true, regAddress, value);
...
{
#ifdef _DEBUG
	logDebug(address, true, regAddress, value, size);
#endif
	...
	if (bytesRead <= size)
	{
		...
		logDebug(adress, true, regAddress, values, bytesRead);
...
{
#ifdef _DEBUG		
			String fullMessage  = String("I2CBase(")
								+ String(address, HEX)
								+ String(") : ") 
								+ message;
...
void I2CBase::logDebug (const byte address, const bool bReading, const byte regAddress, const byte* values, const uint32_t size)
{
	...
	if (size > 1)
	{
		...
		for (int item = 0, byte* pCurrent = values; item < size; ++item, ++pCurrent)
		{
